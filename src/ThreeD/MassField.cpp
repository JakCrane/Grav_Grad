    ...
    for (int i = 0; i < m_Nx; ++i) {
        for (int j = 0; j < m_Ny; ++j) {
            for (int k = 0; k < m_Nz; ++k) {
                ...
                double z = m_z_min + k * (m_z_max - m_z_min) / (m_Nz - 1);
                if (std::abs(x - m_current_position[0]) < m_side_length_x / 2 &&
                    ...
                    m_domain[(i * m_Ny + j) * m_Nz + k] = m_density * m_dx * m_dy * m_dz;
                }
            }
        }
    }
    ...
    for (int i = 0; i < m_Nx; ++i) {
        for (int j = 0; j < m_Ny; ++j) {
            for (int k = 0; k < m_Nz; ++k) {
                ...
                m_domain[i * m_Ny + j * m_Nz + k] = m_density * m_dx * m_dy * m_dz * transition;
            }
        }
    }
}